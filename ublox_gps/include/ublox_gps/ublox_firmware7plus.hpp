...
class UbloxFirmware7Plus : public UbloxFirmware {
 ...
 protected:
  ...
  void fixDiagnostic(diagnostic_updater::DiagnosticStatusWrapper& stat) override {
    ...
    if (last_nav_pvt_.flags & ublox_msgs::msg::NavPVT::FLAGS_DIFF_SOLN) {
      stat.message += ", DGNSS";
    }
    // If DGNSS, then update the differential solution status
    if (last_nav_pvt_.flags & ublox_msgs::msg::NavPVT::CARRIER_PHASE_FLOAT) {
      stat.message += ", FLOAT FIX";
    } else if (last_nav_pvt_.flags & ublox_msgs::msg::NavPVT::CARRIER_PHASE_FIXED) {
